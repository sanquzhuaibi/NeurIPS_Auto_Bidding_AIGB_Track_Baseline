...
class Trainer:
    ...
    def train_iteration(self, num_steps, logger, iter_num=0, log_writer=None, log_path=None):
        ...
        for i in trange(num_steps):
            ...
            if i % 1000 == 0:
                print(f"Step: {i + iter_num*num_steps} BC Loss: {loss_metric['bc_loss'][i]}")
                print(f"Step: {i+ iter_num*num_steps} QL Loss: {loss_metric['ql_loss'][i]}")
                print(f"Step: {i+ iter_num*num_steps} Actor Loss: {loss_metric['actor_loss'][i]}")
                print(f"Step: {i+ iter_num*num_steps} Critic Loss: {loss_metric['critic_loss'][i]}")
                print(f"Step: {i+ iter_num*num_steps} Target Q Mean: {loss_metric['target_q_mean'][i]}")
                if i % 10000 == 0:
                    save_path = f"{log_path}/saved_model/QTtest"
                    ...
                    if not os.path.exists(save_path):
                        os.makedirs(save_path)
                    file_path = os.path.join(save_path, "critic.pt")
                    torch.save(self.critic.state_dict(), file_path)

            log_writer.add_scalar('BC Loss', loss_metric['bc_loss'][i], i+ iter_num*num_steps)
            log_writer.add_scalar('QL Loss', loss_metric['ql_loss'][i], i+ iter_num*num_steps)
            log_writer.add_scalar('Actor Loss', loss_metric['actor_loss'][i], i+ iter_num*num_steps)
            log_writer.add_scalar('Critic Loss', loss_metric['critic_loss'][i], i+ iter_num*num_steps)
            log_writer.add_scalar('Target Q Mean', loss_metric['target_q_mean'][i], i+ iter_num*num_steps)
