...
class EpisodeReplayBuffer(Dataset):
    def __init__(self, state_dim, act_dim, data_path, max_ep_len=24, scale=2000, K=10):
        ...
        print(f"reward_continuous_max: {training_data['reward_continuous'].values.max()}")
        print(f"reward__max: {training_data['reward'].values.max()}")
        ...
        for index, row in self.trajectories.iterrows():
            ...
            if row["done"]:
                if len(state) != 1:#如果个trajectory出现多个done，那么后面的done的信息就不需要了
                    self.states.append(np.array(state))
                    reward_arr =   np.array(reward)
                    cpa_real = all_cost/(np.array(reward).sum() + 1e-10)
                    score = getScore_nips(np.array(reward).sum(), cpa_real, row['CPAConstraint'])
                    coef = score / (np.array(reward).sum() + 1e-10) #乘以coef就是


                    #terminal cost
                    reward[-1] = reward[-1] + (score - np.array(reward).sum())
                    ...
                    #平到到每个reward
                    # self.rewards.append(np.expand_dims(np.array(reward)*coef, axis=1))
                    # self.actions.append(np.expand_dims(np.array(action), axis=1))
                    # self.returns.append(sum(reward)*coef)
...
class EpisodeReplayBufferIQL(Dataset):
    def __init__(self, state_dim, act_dim, data_path, max_ep_len=24, scale=2000, K=20):
        self.device = "cpu"
        super(EpisodeReplayBufferIQL, self).__init__()
        self.max_ep_len = max_ep_len
        self.scale = scale
        ...
        for index, row in self.trajectories.iterrows():
            ...
            if row["done"] == 0:
                ...
                next_state.append((0,)*len(row["state"]))
            if row["done"]:
                if len(state) != 1: #如果长度是1，那么就是无效(s,a,r,s')
                    self.states.append(np.array(state))
                    self.rewards.append(np.expand_dims(np.array(reward), axis=1))
                    self.actions.append(np.expand_dims(np.array(action), axis=1))
                    self.returns.append(sum(reward))
        ...
        self.scale = np.max(tmp_rewards,axis=0)
        ...
        self.p_sample = self.traj_lens[self.sorted_inds] / sum(self.traj_lens[self.sorted_inds])#控制每个样本被选中的概率，避免过拟合
    ...
    def __getitem__(self, index):
        ...
        timesteps = np.arange(start_t, start_t + s.shape[0])
        timesteps[timesteps >= self.max_ep_len] = self.max_ep_len - 1  # padding cutoff
        rtg = self.discount_cumsum(traj['rewards'][start_t:]/self.scale, gamma=1.)[:s.shape[0] + 1].reshape(-1, 1)
        if rtg.shape[0] <= s.shape[0]:
            rtg = np.concatenate([rtg, np.zeros((1, 1))], axis=0)
...
class EpisodeReplayBufferIQLMinMax(Dataset):
    def __init__(self, state_dim, act_dim, data_path, max_ep_len=24, scale=2000, K=20):
        self.device = "cpu"
        super(EpisodeReplayBufferIQLMinMax, self).__init__()
        self.max_ep_len = max_ep_len
        self.scale = scale
    ...
    def __getitem__(self, index):
        ...
        tlen = s.shape[0]
        s = np.concatenate([np.zeros((self.K - tlen, self.state_dim)), s], axis=0)
        s = (self.state_max - s ) / (self.state_max - self.state_min)
        s_prime = np.concatenate([np.zeros((self.K - tlen, self.state_dim)), s_prime], axis=0)
        s_prime (self.state_max - s_prime ) / (self.state_max - self.state_min)
        a = np.concatenate([np.ones((self.K - tlen, self.act_dim)) * -10., a], axis=0)
        r = np.concatenate([np.zeros((self.K - tlen, 1)), r], axis=0)
        r = r / self.scale
        d = np.concatenate([np.ones((self.K - tlen)) * 2, d], axis=0)
