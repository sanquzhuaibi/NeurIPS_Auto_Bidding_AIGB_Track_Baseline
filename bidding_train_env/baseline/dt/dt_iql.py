...
class DecisionTransformerIQL(nn.Module):
    ...
    def __init__(self, dim_obs=3, dim_actions=1, gamma=0.99, tau=0.01, V_lr=5e-4, critic_lr=5e-4, actor_lr=5e-4,
                 network_random_seed=1, expectile=0.7, temperature=3.0, state_mean=0, state_std=1, scale=1):
        super().__init__()
        ...
        self.value_net = V(self.num_of_states).to(self.device)
        self.critic1 = Q(self.num_of_states, self.num_of_actions).to(self.device)
        self.critic2 = Q(self.num_of_states, self.num_of_actions).to(self.device)
        self.critic1_target = Q(self.num_of_states, self.num_of_actions).to(self.device)
        self.critic1_target.load_state_dict(self.critic1.state_dict())
        self.critic2_target = Q(self.num_of_states, self.num_of_actions).to(self.device)
        ...
        self.actors = DecisionTransformer(state_dim=dim_obs, act_dim=dim_actions, state_mean=state_mean,
                                state_std=state_std, scale=scale).to(self.device)
        self.actors.device = self.device
        self.GAMMA = gamma
        self.tau = tau
    ...
    def save_net(self, save_path):
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        file_path = os.path.join(save_path, "dtiql.pt")
...
class DecisionTransformer(nn.Module):
    ...
    def __init__(self, state_dim, act_dim, state_mean, state_std, action_tanh=False, K=30, max_ep_len=96, scale=1,
                 target_return=4):
        ...
        self.scale = scale
        self.target_return = target_return/self.scale
    ...
    def evaluate(self, state, target_return=None, pre_reward=None):
        ...
        if self.eval_states is None:
            ...
            self.eval_rewards[-1] = pre_reward / self.scale
    ...
    def take_actions(self, state, target_return=None, pre_reward=None):
        ...
        action = self.get_action(
            (self.eval_states.to(dtype=torch.float32) -torch.from_numpy( self.state_mean).to(self.device)) / torch.from_numpy(self.state_std).to(self.device),
