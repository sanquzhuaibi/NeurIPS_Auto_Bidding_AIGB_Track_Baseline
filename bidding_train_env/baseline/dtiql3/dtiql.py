...
class DecisionTransformerIQL(nn.Module):
    ...
    def __init__(self, dim_obs=3, dim_actions=1, gamma=0.99, tau=0.01, V_lr=5e-4, critic_lr=5e-4, actor_lr=5e-4,
                 network_random_seed=3407, expectile=0.7, temperature=3.0, state_mean=0, state_std=1,
                 state_max=0, state_min=1, reward_max=0, reward_min=1):
        super().__init__()
        ...
        self.actors = DecisionTransformer(state_dim=dim_obs, act_dim=dim_actions, state_mean=state_mean,
                                state_std=state_std, state_max=state_max, state_min=state_min, reward_max=reward_max, reward_min=reward_min)
        self.GAMMA = gamma
        self.tau = tau
...
class DecisionTransformer(nn.Module):
    ...
    def __init__(self, state_dim, act_dim, state_mean, state_std, state_max, state_min, reward_max, reward_min, action_tanh=False, K=30, max_ep_len=96, scale=2000,
                 target_return=4):
        ...
        # )
        self.predict_action = Actor(self.hidden_size, act_dim)
    ...
    def forward(self, states, actions, rewards, returns_to_go, timesteps, attention_mask=None):
        ...
        action_preds = state_preds


        # mu = self.FC_mu(x[:, 0])
        # log_std = self.FC_std(x[:, 0])
        mu, log_std = self.predict_action(x[:, 0])
        # log_std = torch.clamp(log_std, self.log_std_min, self.log_std_max)
    ...
    def evaluate(self, state, target_return=None, pre_reward=None):
        ...
        if self.eval_states is None:
            ...
            self.eval_rewards[-1] = (pre_reward-self.reward_min)/(self.reward_max -self.reward_min)
        ...
        # )
        action = self.get_action(
            (self.eval_states.to(dtype=torch.float32) - self.state_min) / (self.state_max - self.state_min),
            self.eval_actions.to(dtype=torch.float32),
            self.eval_rewards.to(dtype=torch.float32),
            self.eval_target_return.to(dtype=torch.float32),
            self.eval_timesteps.to(dtype=torch.long)
    ...
    def take_actions(self, state, target_return=None, pre_reward=None):
        ...
        action = self.get_action(
            (self.eval_states.to(dtype=torch.float32, device=self.device) - torch.tensor(self.state_min).to(self.device)) / (torch.tensor(self.state_max).to(self.device) - torch.tensor(self.state_min).to(self.device)),
            self.eval_actions.to(dtype=torch.float32, device=self.device),
            self.eval_rewards.to(dtype=torch.float32, device=self.device),
            self.eval_target_return.to(dtype=torch.float32, device=self.device),
            self.eval_timesteps.to(dtype=torch.long, device=self.device)
