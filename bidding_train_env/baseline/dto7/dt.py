...
class DecisionTransformer(nn.Module):
    ...
    def __init__(self, state_dim, act_dim, state_mean, state_std, state_max, state_min, action_tanh=False, K=10, max_ep_len=96, scale=2000,
        ...
        self.predict_action =  Actor(self.hidden_size, self.act_dim, 200)
    ...
    def forward(self, states, actions, rewards, returns_to_go, costs, ctg,  timesteps, attention_mask=None):
        ...
        stacked_inputs = torch.stack(
            (ctg_embeddings, returns_embeddings, state_embeddings, action_embeddings), dim=1
    ...
    def get_action(self, states, actions, rewards, returns_to_go, costs, ctg, timesteps, **kwargs):
        ...
        _, action_preds, return_preds, reward_preds = self.forward(
            states, actions, rewards, returns_to_go, costs, costs_to_go, timesteps, attention_mask=attention_mask, **kwargs)

        return action_preds[0, -1]

    def step(self, states, actions, rewards, dones, rtg, timesteps, attention_mask, costs, ctg):
        states = states.cuda()
        ...
        state_preds, action_preds, return_preds, reward_preds = self.forward(
            states, actions, rewards, rtg[:, :-1], costs, ctg[:, :-1], timesteps, attention_mask=attention_mask,
        )
