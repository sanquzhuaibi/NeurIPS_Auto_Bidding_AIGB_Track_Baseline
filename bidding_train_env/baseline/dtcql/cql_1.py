...
class CQL(nn.Module):
    def __init__(self, num_inputs, act_dim, device, config=False):
        super().__init__()
        ...
        self.policy_type = "Deterministic",  # "Gaussian", "Deterministic"
        self.target_update_interval = 1  # 1
        self.automatic_entropy_tuning = False # False
        self.device = device  # cuda or cpu
        self.Conservative_Q =int(3) # int(0), int(1), int(2), int(3)

        self.critic = QNetwork(num_inputs, act_dim, 256).to(device=self.device)  # 256
        self.critic_optim = Adam(self.critic.parameters(), lr=0.0003)  # 0.0003
        self.critic_target = QNetwork(num_inputs, act_dim, 256).to(self.device)
        ...
        if self.policy_type == "Gaussian":
            ...
            self.policy = GaussianPolicy(num_inputs, act_dim, 256, act_dim, True)
            ...
            self.policy = DeterministicPolicy(num_inputs, act_dim, 256, None)
            self.policy.to(self.device)
            self.policy_optim = Adam(self.policy.parameters(), lr=256)
    ...
    def step(self, state_batch, action_batch, reward_batch, next_state_batch, mask_batch, batch_size):
        ...
        if self.Conservative_Q == 0: # Core Q0 No Conservative Term
            ...
            cql_weight = 5.
            with torch.no_grad():
                cql_random_actions = next_state_action.new_empty((batch_size, self.act_dim, SP_act), requires_grad=False).uniform_(-1, 1)
                cql_current_actions = next_state_action.new_empty((batch_size, self.act_dim, SP_act), requires_grad=False)
                cql_current_log_pis = next_state_action.new_empty((batch_size, self.act_dim, SP_act), requires_grad=False)
                cql_next_actions = next_state_action.new_empty((batch_size, self.act_dim, SP_act), requires_grad=False)
                cql_next_log_pis = next_state_action.new_empty((batch_size, self.act_dim, SP_act), requires_grad=False)
