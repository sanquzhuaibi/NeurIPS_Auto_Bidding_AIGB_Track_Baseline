"""分数:0.2946      step_num = 20000  batch_size = 128"""
...
class DecisionTransformer(nn.Module):
    ...
    def __init__(self, state_dim, act_dim, state_mean, state_std, state_norm_dict, action_tanh=False, K=10, max_ep_len=96, scale=2000,
                 target_return=4):
        ...
        self.state_norm_dict = state_norm_dict
        self.state_norm_df = pd.DataFrame(self.state_norm_dict).T
        self.state_norm_min_arr =np.array(pd.DataFrame(self.state_norm_dict).T['min'])
        self.state_norm_max_arr =np.array(pd.DataFrame(self.state_norm_dict).T['max'])
        ...
        self.critic = Q(self.state_dim, self.act_dim)
        self.value_net =  V(self.state_dim)
        ...
        self.transformer = nn.ModuleList([Block(self.block_config) for _ in range(self.block_config['n_layer'])])
    ...
    def step(self, states, actions, rewards, dones, rtg, timesteps, attention_mask):
        ...
        rewards_target, action_target, rtg_target, states_target = torch.clone(rewards), torch.clone(actions), torch.clone(rtg), torch.clone(states)
        ...
        states_target = states_target.reshape(-1, state_dim)[attention_mask.reshape(-1) > 0]


        rtg_dim = rtg_target.shape[-1]
        rtg_target = rtg_target[:,:-1,:].reshape(-1, rtg_dim)[attention_mask.reshape(-1) > 0]

        loss = torch.mean((action_preds - action_target) ** 2)
        state_loss = torch.mean((state_preds - states_target) ** 2)
        # value_loss = torch.mean((self.value_net(state_preds) - rtg_target) ** 2)
        # loss_all = loss + state_loss + 2.5*value_loss
        # loss_all = loss + 2.5*value_loss
    ...
    def take_actions(self, state, target_return=None, pre_reward=None):
        ...
        if self.eval_states is None:
            ...
            pred_return = self.eval_target_return[0, -1] - (pre_reward )
        ...
        #     (self.eval_states.to(dtype=torch.float32).to(self.device) - torch.from_numpy(self.state_mean).to(self.device)) / torch.from_numpy(self.state_std).to(self.device),
        ...
        # )
        action = self.get_action(
            (torch.from_numpy(self.state_norm_max_arr).to(self.device) - self.eval_states.to(dtype=torch.float32).to(self.device)) /
            (torch.from_numpy(self.state_norm_max_arr).to(self.device) - torch.from_numpy(self.state_norm_min_arr).to(self.device) + 1e-10),
            self.eval_actions.to(dtype=torch.float32).to(self.device) ,
            self.eval_rewards.to(dtype=torch.float32).to(self.device) ,
            self.eval_target_return.to(dtype=torch.float32).to(self.device) ,
            self.eval_timesteps.to(dtype=torch.long).to(self.device),
