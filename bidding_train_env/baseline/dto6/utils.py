...
class EpisodeReplayBufferModify(Dataset):
    def __init__(self, state_dim, act_dim, data_path, state_max=None, state_min=None, max_ep_len=24, scale=2000, K=20):
        self.device ='cuda' if torch.cuda.is_available() else 'cpu'
        ...
        for index, row in self.trajectories.iterrows():
            ...
            cost.append(cost_temp)
            training_data.loc[index,"cost"]  = cost_temp
            score_temp, penalty_temp = getScore_nips(sum(reward), cost_temp / (sum(reward)+1e-19), row['CPAConstraint'])
            penalty.append(penalty_temp)
            score.append(score_temp)
            training_data.loc[index,"penalty"] = penalty_temp
            training_data.loc[index,"score"]   = score_temp
            if index % 48 == 0 :
                ...
                if len(cost) != 1: #如果没到48到done了
                    cost_cur.append(max(0,cost_temp - cost[-2]))#当前刻刻的cost减去上个时刻的累计cost
                    cost_constrain.append(cost_cur[-1] * 2 -  row['CPAConstraint'] * reward[-1] - 1/48 * row['budget'])
                else:
                    cost_cur.append(0)
                    cost_constrain.append(- 1/48 * row['budget'])
            ...
            if row["done"]:
                if len(state) != 1:
                    assert sum(cost_cur) - row['realAllCost'] <= 1e-5
                    state_arr = np.array(state)
                    state_dim_temp = state_dim - 3
                    diff = state_arr[:, :state_dim_temp//2] - state_arr[:, state_dim_temp//2:state_dim_temp]
                    state_arr[:, state_dim_temp // 2:state_dim_temp] = diff
                    self.states.append(state_arr)
                    self.rewards.append(np.expand_dims(np.array(reward), axis=1)) #Note rewards进行了衰减
                    self.actions.append(np.expand_dims(np.array(action), axis=1))
                    self.returns.append(sum(reward))
                    self.traj_lens.append(len(state))
                    self.dones.append(np.array(dones))
        ...
        self.state_mean, self.state_std = np.mean(tmp_states, axis=0), np.std(tmp_states, axis=0) + 1e-10
        if state_max is None and state_min is None:
            self.state_max, self.state_min = np.max(tmp_states, axis=0)*1.2, np.min(tmp_states, axis=0)/1.2 + 1e-10
        else:
        ...
        for i in range(len(self.states)):
            ...
            self.trajectories.append(
                {"observations": self.states[i], "actions": self.actions[i], "rewards": self.rewards[i],
                 "dones": self.dones[i], "costs": self.costs_constrain[i], "costs_cur": self.costs_cur[i],
                 "CPAconstrain":self.cpa_constrain[i], "coef":self.penalties_last[i], "return": self.returns[i]})
    ...
    def __getitem__(self, index):
        ...
        c = traj['costs'][start_t: start_t + self.K].reshape(-1, 1) #for constrain
        c_cur = traj['costs_cur'][start_t: start_t + self.K].reshape(-1, 1) #for current cost of budget
        ...
        ctg = self.discount_cumsum(traj['costs'][start_t:], gamma=1.)[:s.shape[0] + 1].reshape(-1, 1)#for  constrain
        if ctg.shape[0] <= s.shape[0]:
            ctg = np.concatenate([ctg, np.zeros((1, 1))], axis=0)


        """++++++++++++++++++++++++++++++++++++++++++"""
        ctg_cur = self.discount_cumsum(traj['costs_cur'][start_t:], gamma=1.)[:s.shape[0] + 1].reshape(-1, 1)#for  budget
        if ctg_cur.shape[0] <= s.shape[0]:
            ctg_cur = np.concatenate([ctg_cur, np.zeros((1, 1))], axis=0)
        cpa_real = ctg_cur / (rtg + 1e-19)
        # coef = getScore_nips(rtg, cpa_real, np.array([traj['CPAconstrain']]*len(cpa_real)))
        beta = 2
        penalty = 1
        coef = np.array([traj['CPAconstrain']]*len(cpa_real)) / (cpa_real + 1e-19)
        coef = np.minimum(coef, 1)
        coef = pow(coef, beta)
        rtg_post = coef * rtg #从terminal往t时刻计算

        coef = np.array([traj['coef']]*len(r))[:,None] #每条trajectory的计算出来的衰减的return
        return_o = np.array([traj['return']]*len(r))[:,None] #每条trajectory的计算出来的衰减的return
        return_c = coef * return_o#算后后的return
        ...
        rtg = np.concatenate([np.zeros((self.K - tlen, 1)), rtg], axis=0) / self.scale
        ctg = np.concatenate([np.zeros((self.K - tlen, 1)), ctg], axis=0) / self.scale
        # ctg_cur = np.concatenate([np.zeros((self.K - tlen, 1)), ctg_cur], axis=0) / self.scale
        coef = np.concatenate([np.zeros((self.K - tlen, 1)), coef], axis=0)
        return_c = np.concatenate([np.zeros((self.K - tlen, 1)), return_c], axis=0)/ self.scale
        return_o = np.concatenate([np.zeros((self.K - tlen, 1)), return_o], axis=0)/ self.scale
        ...
        c = torch.from_numpy(c).to(dtype=torch.float32, device=self.device)
        ctg = torch.from_numpy(ctg).to(dtype=torch.float32, device=self.device)
        coef = torch.from_numpy(coef).to(dtype=torch.float32, device=self.device)
        return_c = torch.from_numpy(return_c).to(dtype=torch.float32, device=self.device)
        return_o = torch.from_numpy(return_o).to(dtype=torch.float32, device=self.device)


        timesteps = torch.from_numpy(timesteps).to(dtype=torch.long, device=self.device)
        mask = torch.from_numpy(mask).to(device=self.device)
        return s, a, r, d, rtg, timesteps, mask, c, ctg, coef, return_o, return_c
