...
class DtBiddingStrategy(BaseBiddingStrategy):
    ...
    def __init__(self, budget=100, name="Decision-Transformer-PlayerStrategy", cpa=2, category=1):
        ...
        model_path = os.path.join(dir_name, "saved_model", "DTO2test", "dt.pt")
        picklePath = os.path.join(dir_name, "saved_model", "DTO2test", "normalize_dict.pkl")
        device = "cuda" if torch.cuda.is_available() else "cpu"

        with open(picklePath, 'rb') as f:
            normalize_dict = pickle.load(f)
        self.model = DecisionTransformer(state_dim=44, act_dim=1, state_mean=normalize_dict["state_mean"],
