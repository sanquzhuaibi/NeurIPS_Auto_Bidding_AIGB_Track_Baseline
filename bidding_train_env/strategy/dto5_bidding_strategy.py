...
from bidding_train_env.baseline.dto5.dt import DecisionTransformer
...
class DtBiddingStrategy(BaseBiddingStrategy):
    ...
    def __init__(self, budget=100, name="Decision-Transformer-PlayerStrategy", cpa=2, category=1):
        ...
        model_path = os.path.join(dir_name, "saved_model", "DTO5test", "dt.pt")
        picklePath = os.path.join(dir_name, "saved_model", "DTO5test", "normalize_dict.pkl")
        device = "cuda" if torch.cuda.is_available() else "cpu"

        with open(picklePath, 'rb') as f:
            normalize_dict = pickle.load(f)
        self.model = DecisionTransformer(state_dim=35, act_dim=1, state_mean=normalize_dict["state_mean"],
                                         state_std=normalize_dict["state_std"], state_max=normalize_dict["state_max"], state_min=normalize_dict["state_min"],
                                         target_return=budget*cpa*1.5)
    ...
    def bidding(self, timeStepIndex, pValues, pValueSigmas, historyPValueInfo, historyBid,
        ...
        self.test_state = np.concatenate((test_state,  test_state - self.test_state_old))
        self.test_state = np.concatenate((self.test_state, np.array([self.category, self.budget, self.cpa])))
        self.test_state_old = test_state.copy()

        if len(history_conversion) != 0:
            self.cost_constraint = 2*self.cost_cur - self.cpa * sum(history_conversion[-1]) - 1/48 * self.budget
        else:
            self.cost_constraint = None

        alpha = self.model.take_actions(self.test_state,
