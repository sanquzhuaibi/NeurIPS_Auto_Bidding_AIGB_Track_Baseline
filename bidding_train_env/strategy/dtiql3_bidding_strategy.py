...
from bidding_train_env.baseline.dtiql3.dtiql import DecisionTransformerIQL as DecisionTransformer
...
class PlayerBiddingStrategy(BaseBiddingStrategy):
    ...
    def __init__(self, budget=100, name="PlayerStrategy", cpa=2, category=1):
        """
        ...
        model_path = os.path.join(dir_name, "saved_model", "DTIQL3test", "dt.pt")
        picklePath = os.path.join(dir_name, "saved_model", "DTIQL3test", "normalize_dict.pkl")

        with open(picklePath, 'rb') as f:
            normalize_dict = pickle.load(f)
        self.model = DecisionTransformer(32, 1, state_mean=normalize_dict["state_mean"],
                                         state_std=normalize_dict["state_std"],
                                         state_max=normalize_dict["state_max"],
                                         state_min=normalize_dict["state_min"])
    ...
    def bidding(self, timeStepIndex, pValues, pValueSigmas, historyPValueInfo, historyBid,
        ...
        if timeStepIndex == 0:
            self.model.actors.init_eval()

        self.test_state = np.concatenate((test_state,  self.test_state_old))
        self.test_state_old = test_state.copy()

        alpha = self.model.actors.take_actions(self.test_state,
                                        pre_reward=sum(history_conversion[-1]) if len(
                                            history_conversion) != 0 else None)
        bids = alpha * pValues
        return bids
