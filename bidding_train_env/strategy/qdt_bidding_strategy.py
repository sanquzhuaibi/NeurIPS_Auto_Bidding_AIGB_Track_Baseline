...
class QDTBiddingStrategy(BaseBiddingStrategy):
    """
    ...
    def __init__(self, budget=100, name="Decision-Transformer-PlayerStrategy", cpa=2, category=1):
        ...
        model_path = os.path.join(dir_name, "saved_model", "QDTtest", "dt.pt")
        picklePath = os.path.join(dir_name, "saved_model", "QDTtest", "normalize_dict.pkl")
        device = "cuda" if torch.cuda.is_available() else "cpu"

        with open(picklePath, 'rb') as f:
            normalize_dict = pickle.load(f)
        self.model = DecisionTransformer(state_dim=16, act_dim=1, state_mean=1,
                                         state_std=0, state_norm_dict=normalize_dict)
