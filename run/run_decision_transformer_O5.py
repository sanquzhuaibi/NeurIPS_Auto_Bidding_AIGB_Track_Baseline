...
from bidding_train_env.baseline.dto5 .utils import EpisodeReplayBufferModify as EpisodeReplayBuffer
...
writer = SummaryWriter(f"results/dto5_{formatted_datetime}")
# 配置日志
logging.basicConfig(
    level=logging.INFO,
...
def train_model():
    ...
    with open(f'results/dto5_{formatted_datetime}/model_hyperparameters.txt', 'w') as f:
    ...
    for states, actions, rewards, dones, rtg, timesteps, attention_mask, c, ctg in dataloader:
        ...
        if i % 1000 == 0:
            ...
            if i % 5000 == 0:
                model.save_net(f"results/dto5_{formatted_datetime}/saved_model/DTO5test/{i}")
                save_normalize_dict({"state_mean": replay_buffer.state_mean, "state_std": replay_buffer.state_std,
                                     "state_max": replay_buffer.state_max, "state_min": replay_buffer.state_min},
                                    f"results/dto5_{formatted_datetime}/saved_model/DTO5test/{i}")
    ...
    model.save_net("saved_model/DTO5test")
