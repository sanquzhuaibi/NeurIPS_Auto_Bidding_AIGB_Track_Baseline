...
def train_cql_model():
    ...
    # Build replay buffer
    replay_buffer =  EpisodeReplayBuffer_QDT2(16, 1, training_data, model=model_pre,is_normalize=is_normalize)
    step_num = 1
    batch_size = 64
...
class EpisodeReplayBuffer_QDT(Dataset):
    def __init__(self, state_dim, act_dim, training_data, max_ep_len=24, scale=2000, K=20, model=None, is_normalize=False):
        self.device = "cpu"
        ...
        for index, row in self.trajectories.iterrows():
            ...
            if is_normalize:
                state.append(row['normalize_state'])
                reward.append(row['reward_continuous'])#reward without normalized
                if row["done"] == 0:
                    next_state.append(row["normalize_nextstate"])
                else:
        ...
        self.state_mean, self.state_std = np.max(tmp_states, axis=0), np.min(tmp_states, axis=0) + 1e-10
    ...
    def __getitem__(self, index):
        ...
        rtg = self.relabelling_rtg(traj['rewards'], gamma=1., obs=traj['observations'], act= traj['actions'])
        ...
        # obs_act = np.concatenate((traj['observations'][start_t:],  traj['actions'][start_t:]),-1)
        # rtg = self.relabelling_rtg(traj['rewards'][start_t:], gamma=1., obs=traj['observations'][start_t:], act= traj['actions'][start_t:])
        if rtg.shape[0] <= s.shape[0]:
            rtg = np.concatenate([rtg, np.zeros((1, 1))], axis=0)

        # self.generate_rtg(traj['rewards'][start_t:], )
        #
        tlen = s.shape[0]
        t = min(start_t + self.K, len(rtg)-1)

        # rtg = np.concatenate([np.zeros((self.K - tlen, 1)), rtg], axis=0) / self.scale
        rtg = self.generate_rtg(r, 1.0, rtg[t])
        rtg = np.concatenate([np.zeros((self.K - tlen, 1)), rtg], axis=0)
        s = np.concatenate([np.zeros((self.K - tlen, self.state_dim)), s], axis=0)
        # s = (self.state_max - s) / (self.state_max - self.state_min + 1e-10)
        s_prime = np.concatenate([np.zeros((self.K - tlen, self.state_dim)), s_prime], axis=0)
        # s_prime =  (self.state_max - s_prime) / (self.state_max - self.state_min + 1e-10)
    ...
    def discount_cumsum(self, x, gamma=1.):
        ...
        for t in reversed(range(x.shape[0] - 1)):
            discount_cumsum[t] = x[t] + gamma * max(discount_cumsum[t + 1])
        return discount_cumsum

    def relabelling_rtg (self, x, gamma, obs, act):
        ...
        v = self.model.critic1(obs, act).detach().cpu().numpy()
        discount_cumsum = np.zeros_like(x)
        discount_cumsum[-1] = x[-1]
        for t in reversed(range(x.shape[0] - 1)):
            discount_cumsum[t] = x[t] + gamma * max(discount_cumsum[t + 1], v[t+1])
        return discount_cumsum

    def generate_rtg (self, x, gamma, R_t):
        discount_cumsum = np.zeros_like(x)
        # discount_cumsum[-1] = x[-1]
        discount_cumsum[-1] = R_t
        for t in reversed(range(x.shape[0] - 1)):
...
class EpisodeReplayBuffer_QDT2(Dataset):
    def __init__(self, state_dim, act_dim, training_data, max_ep_len=24, scale=2000, K=5, model=None, is_normalize=False):
        self.device = "cpu"
        super(EpisodeReplayBuffer_QDT2, self).__init__()
        self.max_ep_len = max_ep_len
        self.scale = scale
        self.model = model
        ...
        for index, row in self.trajectories.iterrows():
            ...
            if is_normalize:
                state.append(row['normalize_state'])
                reward.append(row['normalize_reward'])
                if row["done"] == 0:
                    next_state.append(row["normalize_nextstate"])
                else:
                    next_state.append((0,) * len(row["state"]))
            ...
            if row["done"] == 1 and row["timeStepIndex"] == 47:
                if len(state) != 1:
                    self.states.append(np.array(state))
                    self.rewards.append(np.expand_dims(np.array(reward), axis=1))
                    self.actions.append(np.expand_dims(np.array(action), axis=1))
    ...
    def __getitem__(self, index):
        traj = self.trajectories[int(self.sorted_inds[index])]
        start_t = random.randint(0, traj['rewards'].shape[0] - 1-self.K)
        rtg = self.relabelling_rtg(traj['rewards'], gamma=1., obs=traj['observations'], act= traj['actions'])
        ...
        #
        tlen = s.shape[0]
        t = min(start_t + self.K-1, len(rtg)-1)
    ...
    def relabelling_rtg (self, x, gamma, obs, act):
        ...
        act = torch.from_numpy(act).to(dtype=torch.float32, device=self.device)
        v = self.model.critic1(obs, act).detach().cpu().numpy()
        v = self.model.value_net(obs).detach().cpu().numpy()
        discount_cumsum = np.zeros_like(x)
        discount_cumsum[-1] = x[-1]
        for t in reversed(range(x.shape[0] - 1)):
            discount_cumsum[t] = x[t] + gamma * max(discount_cumsum[t + 1], v[t+1])
