...
from bidding_train_env.baseline.dto4 .utils import EpisodeReplayBuffer
...
writer = SummaryWriter(f"results/dto4_{formatted_datetime}")
# 配置日志
logging.basicConfig(
    level=logging.INFO,
...
def train_model():
    ...
    with open(f'results/dto4_{formatted_datetime}/model_hyperparameters.txt', 'w') as f:
    ...
    for states, actions, rewards, dones, rtg, timesteps, attention_mask in dataloader:
        ...
        if i % 1000 == 0:
            ...
            if i % 10000 == 0:
                model.save_net(f"results/dto4_{formatted_datetime}/saved_model/DTO4test")
                save_normalize_dict({"state_mean": replay_buffer.state_mean, "state_std": replay_buffer.state_std},
                                    f"results/dto4_{formatted_datetime}/saved_model/DTO4test")
    ...
    model.save_net("saved_model/DTO4test")
