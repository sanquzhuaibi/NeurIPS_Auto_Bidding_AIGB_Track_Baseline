...
from bidding_train_env.baseline.dto1.utils import EpisodeReplayBuffer
from bidding_train_env.baseline.dto1.dt import DecisionTransformer
...
writer = SummaryWriter(f"results/dto1_{formatted_datetime}")
# 配置日志
logging.basicConfig(
    level=logging.INFO,
...
def train_model():
    ...
    with open(f'results/dto1_{formatted_datetime}/model_hyperparameters.txt', 'w') as f:
    ...
    for states, actions, rewards, dones, rtg, timesteps, attention_mask in dataloader:
        ...
        if i % 1000 == 0:
            ...
            if i % 10000 == 0:
                model.save_net(f"results/dto1_{formatted_datetime}/saved_model/DTO1test")
    ...
    model.save_net("saved_model/DTO1test")
