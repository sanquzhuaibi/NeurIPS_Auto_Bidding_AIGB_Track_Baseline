...
from bidding_train_env.baseline.dtiql.utils import EpisodeReplayBuffer as EpisodeReplayBuffer
from bidding_train_env.baseline.dtiql.dtiql import DecisionTransformerIQL
...
writer = SummaryWriter(f"results/dtiql_{formatted_datetime}")
...
def train_model():
    ...
    model.hyperparameters['step_num'] = step_num
    model.hyperparameters['batch_size'] = batch_size
    with open(f'results/dtiql_{formatted_datetime}/model_hyperparameters.txt', 'w') as f:
    ...
    for states, actions, rewards, dones, rtg, timesteps, attention_mask, next_state in dataloader:
        ...
        if i % 1000 == 0:
            ...
            if i % 10000 == 0:
                model.save_net(f'results/dtiql_{formatted_datetime}/DTIQLtest')
        i += 1
    ...
    model.save_net("saved_model/DTIQLtest")

    # model.save_jit('saved_model/DTIQLtest')
