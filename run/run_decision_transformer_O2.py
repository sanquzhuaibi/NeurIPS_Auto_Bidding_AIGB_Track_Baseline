#0.3770,将上个个时刻的state与当前state作差


import numpy as np
from bidding_train_env.common.utils import normalize_state, normalize_reward, save_normalize_dict
from bidding_train_env.baseline.dto2.utils import EpisodeReplayBuffer
from bidding_train_env.baseline.dto2.dt import DecisionTransformer
...
writer = SummaryWriter(f"results/dto2_{formatted_datetime}")
# 配置日志
logging.basicConfig(
    level=logging.INFO,
...
def train_model():
    ...
    replay_buffer = EpisodeReplayBuffer(state_dim, 1, "./bidding_train_env/data/trajectory/trajectory_data.csv")
    save_normalize_dict({"state_mean": replay_buffer.state_mean, "state_std": replay_buffer.state_std},
                        "saved_model/DTO2test")
    ...
    with open(f'results/dto2_{formatted_datetime}/model_hyperparameters.txt', 'w') as f:
    ...
    for states, actions, rewards, dones, rtg, timesteps, attention_mask in dataloader:
        ...
        if i % 1000 == 0:
            ...
            if i % 10000 == 0:
                model.save_net(f"results/dto2_{formatted_datetime}/saved_model/DTO2test")
                save_normalize_dict({"state_mean": replay_buffer.state_mean, "state_std": replay_buffer.state_std},
                                    "saved_model/DTO2test")
    ...
    model.save_net("saved_model/DTO2test")
